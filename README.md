...
- **Pixel dropdowns.** Every menu is drawn by the studio rather than the operating system, so the theme holds everywhere. `neo-select.js` enhances each `<select>` and keeps the real element in the DOM, so scripts that read or set `.value`, rebuild options, or disable a choice all keep working. Full keyboard support: arrows, Home/End, type-to-jump, Enter to choose, Escape to close.
...
- A master **effects rack** of eight units in one chain: Equaliser, Compressor, Phaser, Flanger, Bitcrush, Reverb, Stereo width and Echo. Each opens in its own draggable window with a live screen and segmented dials (38 in total). Frequency dials travel in octaves.
  - **Equaliser** is a real four-band: low shelf, two bells, high shelf, each with frequency, gain and Q. Its screen draws the combined curve over a live spectrum, and the four numbered handles can be dragged directly.
  - **Compressor** has threshold, ratio, knee, attack, release and makeup. Its screen draws the actual transfer curve and a segmented gain-reduction meter fed by the live compressor.
  - **Stereo** is a full imaging unit: width, bass mono (side content below a chosen frequency is centred), image rotation and balance. Its screen is a goniometer with L/R peak meters and a correlation meter, so you can see whether the mix survives being folded to mono.
  - The other units show what they are doing: sweeping phaser notches, the flanger comb, the bitcrusher's real quantisation staircase, reverb decay taps and tempo-locked echo repeats.
- **Motion** on any automatable dial: a tempo-synced LFO (sine, triangle, square or saw) from 1/4 bar to 8 bars, with adjustable depth. Rack settings live in the pattern, so they save, share and export; motion runs identically in playback and in the offline MP3/WAV render.
- **Voice presets**, 23 of them across five hardware families (N3S pulse & triangle, GB01 handheld, SUPA 16-bit, SAGA FM, SIDX filtered). A preset sets the voice's instrument and effects and nothing else, so any preset works on any chip. The picker shows the matching preset, or "custom" once you move a dial. Presets are built from documented hardware behaviour and tracker convention: duty cycles, the 60 Hz arpeggio trick, FM operator ratios, filter sweeps, delayed vibrato. They are our own settings for our own synth.

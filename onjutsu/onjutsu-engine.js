...
(function (global) {
  ...
  const cfg = {
    ...
    temperature: 1.15,
    topP: 0.96,
    /* Drums get the melodic temperature but a NARROWER nucleus, and the
     * narrowing is the whole point. REST_D is strongly peaked, so at 0.96 the
     * tail of the nucleus is a low-probability drum hit at almost every step,
     * and 19% of generations came out with a hit on ~126 of 128 steps -- ten
     * hits a second of noise channel over everything else. Measured on
     * out_form, 16 patterns per cell, against a corpus drummed density 0.598:
     *
     *     dT 1.15  dP 0.96   density 0.652   saturated 0.188
     *     dT 1.00  dP 0.96   density 0.629   saturated 0.125
     *     dT 1.15  dP 0.92   density 0.579   saturated 0.000
     *     dT 0.90  dP 0.96   density 0.587   saturated 0.000
     *
     * Lowering the TEMPERATURE also fixes it and was tried first on an earlier
     * checkpoint, where it halved drum variety and was reverted. Narrowing
     * top-p costs no variety here (0.031 against 0.029) and addresses the
     * actual mechanism, so it is the one that ships. sample.py carries the
     * same value; if these two ever drift the browser stops sounding like the
     * thing that was auditioned, which is exactly how the last bug happened. */
